...
export default function Alerts() {
  ...
  return (
    <div className="page-enter">
      <PageIntro eyebrow="Clinical signal queue" title="Clinical alerts" description="Review new signals, close the loop, and keep high-risk patients visible." action={<div className="flex items-center gap-2 rounded-lg bg-[hsl(13_75%_92%)] px-3 py-2 text-xs font-bold text-[hsl(13_64%_39%)]"><Bell className="h-4 w-4" /> {unread} unread</div>} />
      <div className="clinical-card rounded-xl p-4 sm:p-5"><div className="flex flex-col justify-between gap-3 sm:flex-row sm:items-center"><SectionTitle title="Active queue" label="Triage board" action={<span className="mono text-[11px] text-muted-foreground">{filtered.length} shown</span>} /><div className="flex gap-1 rounded-lg bg-muted p-1"><FilterButton value="all" active={filter === 'all'} onClick={() => setFilter('all')} label="All" /><FilterButton value="unread" active={filter === 'unread'} onClick={() => setFilter('unread')} label="Unread" /><FilterButton value="high" active={filter === 'high'} onClick={() => setFilter('high')} label="High severity" /></div></div></div>
      <div className="mt-5">
