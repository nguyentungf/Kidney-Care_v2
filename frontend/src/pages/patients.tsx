...
export default function Patients() {
  ...
  return (
    <div className="page-enter">
      <PageIntro eyebrow="Sổ theo dõi" title="Danh sách bệnh nhân" description="Tìm theo tên hoặc mã bệnh nhân" action={<button onClick={() => setCreateOpen(true)} data-testid="button-create-patient" className="inline-flex h-10 items-center justify-center gap-2 rounded-lg bg-primary px-4 text-sm font-bold text-primary-foreground shadow-sm transition hover:-translate-y-0.5 hover:bg-primary/90"><Plus className="h-4 w-4" /> Thêm bệnh nhân</button>} />
      <div className="clinical-card rounded-xl p-4 sm:p-5">
        <div className="flex flex-col gap-3 lg:flex-row"><div className="flex-1"><SearchField value={search} onChange={setSearch} placeholder="Tìm theo tên hoặc mã bệnh nhân" testId="input-patient-search" /></div><div className="flex gap-1 overflow-x-auto rounded-lg bg-muted p-1">{(['all', 'stable', 'watch', 'critical'] as const).map((item) => <button key={item} onClick={() => setStatus(item)} data-testid={`button-filter-${item}`} className={`whitespace-nowrap rounded-md px-3 py-2 text-xs font-bold capitalize transition ${status === item ? 'bg-card text-foreground shadow-sm' : 'text-muted-foreground hover:text-foreground'}`}>{item === 'all' ? 'Tất cả' : item}</button>)}</div></div>
      </div>
      <div className="mt-5">
      ...
      <Modal open={createOpen} onClose={() => setCreateOpen(false)} title="Thêm bệnh án" description="Create a record for the active renal care census.">
        <form onSubmit={submit} className="space-y-4">
          <div className="grid gap-4 sm:grid-cols-2"><FormField label="Họ và tên"><input required minLength={2} data-testid="input-patient-name" value={form.fullName} onChange={(e) => updateField('fullName', e.target.value)} className="form-input" placeholder="VD: Nguyễn Văn A" /></FormField><FormField label="Ngày sinh"><input required type="date" data-testid="input-patient-dob" value={form.dateOfBirth} onChange={(e) => updateField('dateOfBirth', e.target.value)} className="form-input" /></FormField></div>
          <div className="grid gap-4 sm:grid-cols-2"><FormField label="Giới tính"><select data-testid="select-patient-gender" value={form.gender} onChange={(e) => updateField('gender', e.target.value)} className="form-input"><option>Nữ</option><option>Nam</option><option>Không muốn tiết lộ</option></select></FormField><FormField label="Số điện thoại"><input required data-testid="input-patient-phone" value={form.phone} onChange={(e) => updateField('phone', e.target.value)} className="form-input" placeholder="(+84)" /></FormField></div>
          <div className="grid gap-4 sm:grid-cols-3"><FormField label="Giai đoạn CKD"><select data-testid="select-patient-stage" value={form.stage} onChange={(e) => updateField('stage', e.target.value)} className="form-input"><option>CKD Stage 1</option><option>CKD Stage 2</option><option>CKD Stage 3</option><option>CKD Stage 4</option><option>CKD Stage 5</option></select></FormField><FormField label="eGFR"><input required type="number" min="0" max="150" data-testid="input-patient-egfr" value={form.egfr} onChange={(e) => updateField('egfr', e.target.value)} className="form-input" /></FormField><FormField label="Trạng thái"><select data-testid="select-patient-status" value={form.status} onChange={(e) => updateField('status', e.target.value)} className="form-input"><option value="stable">Ổn định</option><option value="watch">Theo dõi</option><option value="critical">Nguy kịch</option></select></FormField></div>
          <div className="grid gap-4 sm:grid-cols-2"><FormField label="Bác sĩ chuyên khoa"><input required data-testid="input-patient-doctor" value={form.doctor} onChange={(e) => updateField('doctor', e.target.value)} className="form-input" /></FormField><FormField label="Địa chỉ"><input data-testid="input-patient-address" value={form.address} onChange={(e) => updateField('address', e.target.value)} className="form-input" placeholder="Phường/Xã, Quận/Huyện, Tỉnh/Thành phố" /></FormField></div>
          <div className="grid gap-4 sm:grid-cols-2"><FormField label="Chẩn đoán"><input data-testid="input-patient-diagnosis" value={form.diagnosis} onChange={(e) => updateField('diagnosis', e.target.value)} className="form-input" /></FormField><FormField label="Dị ứng"><input data-testid="input-patient-allergies" value={form.allergies} onChange={(e) => updateField('allergies', e.target.value)} className="form-input" placeholder="VD: Hải sản" /></FormField></div>
          <div className="flex justify-end gap-2 border-t border-border pt-5"><button type="button" onClick={() => setCreateOpen(false)} data-testid="button-cancel-patient" className="rounded-lg px-4 py-2.5 text-sm font-bold text-muted-foreground hover:bg-muted">Hủy</button><button disabled={create.isPending} data-testid="button-submit-patient" className="rounded-lg bg-primary px-4 py-2.5 text-sm font-bold text-primary-foreground disabled:opacity-50">{create.isPending ? 'Đang lưu...' : 'Lưu'}</button></div>
        </form>
      </Modal>
