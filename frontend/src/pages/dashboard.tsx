...
export default function Dashboard() {
  ...
  if (query.isLoading) return <><PageIntro eyebrow="Good morning, Dr. Sen" title="Clinical overview" description={`Your renal care workspace for ${today}.`} /><StateBlock kind="loading" title="Loading the census" detail="Pulling the latest care activity and risk distribution." /></>;
  ...
  return (
    <div className="page-enter">
      <PageIntro eyebrow={today} title="Good morning, Dr. Sen" description="A concise view of the patients and decisions needing your attention today." action={<Link href="/patients" data-testid="link-dashboard-patients" className="inline-flex h-10 items-center justify-center gap-2 rounded-lg bg-primary px-4 text-sm font-bold text-primary-foreground shadow-sm transition hover:-translate-y-0.5 hover:bg-primary/90"><UsersRound className="h-4 w-4" /> Open registry</Link>} />
      <div className="grid grid-cols-2 gap-3 lg:grid-cols-4">
        <div className="soft-rise"><MetricCard label="Total census" value={summary.totalPatients} detail="active patients" accent="teal" icon={<UsersRound className="h-4 w-4" />} /></div>
        <div className="soft-rise delay-1"><MetricCard label="Critical review" value={summary.criticalPatients} detail="need attention" accent="coral" icon={<AlertTriangle className="h-4 w-4" />} /></div>
        <div className="soft-rise delay-2"><MetricCard label="Dialysis today" value={summary.dialysisToday} detail="sessions booked" accent="gold" icon={<CalendarClock className="h-4 w-4" />} /></div>
        <div className="soft-rise delay-3"><MetricCard label="Unread alerts" value={summary.unreadAlerts} detail="clinical signals" accent="ink" icon={<Activity className="h-4 w-4" />} /></div>
      </div>
      <div className="mt-6 grid gap-6 xl:grid-cols-[1.05fr_.95fr]">
        <section className="clinical-card rounded-xl p-5 sm:p-6">
          <SectionTitle label="Risk distribution" title="Patients by CKD stage" action={<span className="mono text-[11px] text-muted-foreground">n = {summary.totalPatients}</span>} />
        ...
        <section className="clinical-card rounded-xl p-5 sm:p-6">
          <SectionTitle label="Operational pulse" title="Today's dialysis" action={<Link href="/dialysis" data-testid="link-dashboard-dialysis" className="text-xs font-bold text-primary hover:underline">View schedule <ChevronRight className="ml-0.5 inline h-3 w-3" /></Link>} />
      ...
      <section className="clinical-card mt-6 rounded-xl p-5 sm:p-6">
        <SectionTitle label="Audit trail" title="Recent activity" action={<span className="inline-flex items-center gap-1.5 text-xs text-muted-foreground"><CircleCheck className="h-3.5 w-3.5 text-primary" /> Live updates</span>} />
